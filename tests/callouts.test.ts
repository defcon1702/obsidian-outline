...
describe('convertCallouts', () => {
  ...
  it('converts a multi-line callout with body', () => {
    const input = ['> [!NOTE] Important', '> First line of body', '> Second line of body'].join(
      '\n'
    );
    const output = convertCallouts(input);
    expect(output).toBe(
  ...
  it('handles multiple sequential callouts separated by blank line', () => {
    const input = ['> [!WARNING]', '> warn text', '', '> [!SUCCESS]', '> success text'].join('\n');
    const output = convertCallouts(input);
  ...
  it('does NOT eat the paragraph following a callout', () => {
    const input = ['> [!NOTE]', '> callout body', 'This is a normal paragraph'].join('\n');
    const output = convertCallouts(input);
    expect(output).toContain(':::info');
  ...
  it('handles callout with blank continuation line', () => {
    const input = ['> [!INFO] Title', '> First paragraph', '>', '> Second paragraph'].join('\n');
    const output = convertCallouts(input);
    expect(output).toContain(':::info');
  ...
  it('produces Outline import format matching Test Collection.md', () => {
    const input = ['> [!WARNING]', '> This is a warning'].join('\n');
    const output = convertCallouts(input);
    expect(output).toBe([':::warning', 'This is a warning', '', ':::'].join('\n'));
  });
